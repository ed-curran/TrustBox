...
export const createWeb5Agent = async (kmsSecretKey: string, environmentName?: string) => {
  ...
  const {AppDataVault, DwnManager, SyncManagerLevel} = await import('@web5/agent')
  const { LevelStore } = await import('@web5/common');
  const {DidResolver, DidIonMethod, DidKeyMethod} = await import ('@web5/dids')
  ...
  const agent = await IdentityAgent.create({appData, dwnManager, syncManager});
...
export const createWeb5 = async (agent: Web5Agent, name: string = 'default') => {
